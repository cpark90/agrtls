...
# ==================== 사용자 설정 ====================
SERIAL_PORT = "COM5"       # 본인의 ESP32 연결 포트로 변경 (예: Linux는 '/dev/ttyUSB0')
...
class UwbVisualizer(QMainWindow):
    ...
    def parse_serial_line(self, line):
        """
        시리얼 문자열에서 숫자(float)들을 추출하는 함수입니다.
        출력 형태가 "Range: 1.25 m, TX: -40.5" 이든 "1.25, -40.5" 이든
        순서대로 숫자 두 개를 찾아냅니다.
        """
        ...
        if len(numbers) >= 2:
            try:
                ...
                val_range = float(numbers[0])
                val_tx = float(numbers[1])
