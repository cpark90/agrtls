...
```
기대 출력: `ALL TESTS PASSED` / `ALL WINDOW TESTS PASSED`.
...
## test_window_modules — window-TDMA 모듈 (docs/ARCHITECTURE_window_tdma.md)
- tag_quality: θ_link 적격성, 1차 품질(=RXP)
- tag_registry: 유효앵커, 충돌(유효앵커 공유), meanQuality, meanSharedQuality
- window_color: 태그→윈도우 컬러링(충돌=유효앵커 공유, 비충돌 재사용), 상한 초과·무자격 → candidate
- window_frame: 2계층(window×anchor-slot) 타이밍/동기
